    ...
    for (int c = 0; c < channels; ++c) {
        ...
        for (int y = 0; y < height; ++y) {
            ...
            for (int x = 1; x < width; ++x) {
                const int enteringX = std::min(x + radius, width - 1);
                const int leavingX = std::max(0, x - 1 - radius);
    ...
    for (int c = 0; c < channels; ++c) {
        ...
        for (int x = 0; x < width; ++x) {
            ...
            for (int y = 1; y < height; ++y) {
                const int enteringY = std::min(y + radius, height - 1);
                const int leavingY = std::max(0, y - 1 - radius);
